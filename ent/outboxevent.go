...
type OutboxEvent struct {
	...
	// 聚合根类型：agent/user/permission/...
	AggregateType string `json:"aggregate_type,omitempty"`
	// 聚合根 ID（字符串形式，兼容链上/链下 ID）
	AggregateID string `json:"aggregate_id,omitempty"`
	// 事件类型：agent.registered / agent.banned / ...
	...
	// 发布状态：0=pending, 1=published, 2=failed, 3=dead
...
func (*OutboxEvent) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case outboxevent.FieldPayload:
			values[i] = new([]byte)
		case outboxevent.FieldStatus, outboxevent.FieldRetryCount:
			values[i] = new(sql.NullInt64)
		case outboxevent.FieldAggregateType, outboxevent.FieldAggregateID, outboxevent.FieldEventType, outboxevent.FieldIdempotencyKey, outboxevent.FieldLastError:
			values[i] = new(sql.NullString)
		case outboxevent.FieldOccurredAt, outboxevent.FieldNextRetryAt:
